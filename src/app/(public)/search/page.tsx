...
import { MOCK_PROPERTIES, NEARBY_NEIGHBORHOODS } from '@/lib/mock-data'
 ...
 * Manages URL parameter syncing, advanced sorting, active chips, and empty state neighborhood suggestions.
 * Completely optimized for React Compiler guidelines by computing properties directly during render.
 */
function SearchPageContent() {
  ...
  // 1. Read parameters directly from URL query during render (Pure & Single Source of Truth)
  const filters: FilterState = {
    rentMin: parseInt(searchParams.get('minRent') || '0'),
    rentMax: parseInt(searchParams.get('maxRent') || '150000'),
    neighborhood: searchParams.get('location') || '',
    houseType: searchParams.get('type') || '',
    waterRating: searchParams.get('water') || '',
    electricityRating: searchParams.get('electricity') || '',
    internetType: searchParams.get('internet') || '',
    securityRating: searchParams.get('security') || '',
    parking: searchParams.get('parking') || '',
    roadType: searchParams.get('roadAccess') || '',
    garbageReliability: searchParams.get('garbage') || '',
    verifiedOnly: searchParams.get('verified') === 'true',
    communityListedOnly: searchParams.get('community') === 'true',
    vacancyOnly: searchParams.get('vacancy') === 'true',
    recentlyUpdated: searchParams.get('recent') === 'true',
  }

  const sortState = searchParams.get('sort') || 'match'

  // Computed error trigger from URL query
  const errorOccurred = filters.neighborhood.toLowerCase() === 'error'

  // Initialize reference timestamp inside a state initializer (Run once, pure)
  const [referenceTime] = React.useState(() => new Date('2026-07-03T09:14:04+03:00').getTime())

  // 2. Sync changes back to URL parameters via router transitions
  ...
  // 3. Filter mock dataset directly during render
  const filteredProperties = MOCK_PROPERTIES.filter((prop) => {
    // Neighborhood Location check
    if (
    ...
    // Recently Updated check (last 14 days relative to frozen referenceTime)
  ...
  // 4. Sort properties directly during render
  const sortedProperties = [...filteredProperties]
  if (sortState === 'rent-low') {
    sortedProperties.sort((a, b) => a.rentMin - b.rentMin)
  } else if (sortState === 'rent-high') {
    sortedProperties.sort((a, b) => b.rentMax - a.rentMax)
  } else if (sortState === 'health') {
    sortedProperties.sort((a, b) => b.healthScore - a.healthScore)
  } else if (sortState === 'reviews') {
    sortedProperties.sort((a, b) => b.reviewCount - a.reviewCount)
  ...
  // 5. Generate active filter chips directly during render
  ...
  // Get nearby suggestions if search location results in empty states directly during render
        ...
        <Container className="max-w-6xl flex flex-col gap-sm">
          ...
          <div className="flex flex-col lg:flex-row gap-sm items-start">
            ...
            <div className="w-full lg:w-[60%] flex flex-col gap-sm">
              ...
              <div className="flex justify-between items-center bg-bg-secondary border border-border-subtle p-xs rounded-symmetric shadow-sm mt-xxs">
                <span className="text-[13px] font-medium text-text-muted">
                  Showing {isPending ? '...' : sortedProperties.length} homes
                </span>
            ...
            <div className="w-full lg:w-[40%] lg:sticky lg:top-[80px]">
              <MapPreview properties={sortedProperties} onSelectProperty={handlePropertySelect} />
            </div>
          </div>
        </Container>
      </main>
